(function () {
  ...
  let MAX_FILE_BYTES = 300 * 1024 * 1024; // updated from /my
  let MAX_SECONDS = 60; // updated from /my
  ...
  async function refresh() {
    ...
    if (data?.max_file_mb)
    if (data?.max_record_seconds) {
      MAX_SECONDS = data.max_record_seconds;
      if (btnRecord) btnRecord.textContent = `Record (max ${data.max_file_mb}s)`;
    }
    renderList(data);
  }

  function renderList(state) {
    const { items, limits, usage} = state;
    const rows = items
      .map((item) => {
        ...
        <div class="mqvm-row" data-id="${item.id}">
          <div class="mqvm-row-meta">
            ...
            <div class="mqvm-sub">Size: ${sizeMB}MB ${
          dur ? " · Duration: " + dur : ""
        }</div>
          </div>
          <div class="mqvm-row-actions">
            ${
              item.url
                ? `<a class="button" href="${item.url}" target="_blank" rel="noopener">View</a>`
                : ""
            }
            <button class="button mqvm-replace" data-id="${
              item.id
            }">Replace</button>
            <button class="button mqvm-delete" data-id="${
              item.id
            }">Delete</button>
          </div>
        </div>
    ...
    const totalCap =
      limits &&
      typeof limits.max_total_mb !== "undefined" &&
      limits.max_total_mb > 0
        ? ` total ${limits.max_total_mb}MB`
        : ``;

    const quota = `
      <div class="mqvm-quota">
        <div>Plan: max videos ${limits.max_videos} Max file upload limit - ${totalCap}</div>
    ...
    listEl.querySelectorAll(".mqvm-replace").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (!f) {
          alert("Choose a video in the file input first, then click Replace.");
          return;
        }
        uploadFile(f, id);
      });
  ...
  async function uploadFile(file, replaceId = null, durationSec = null) {
    ...
    if (file.size > MAX_FILE_BYTES) {
      alert(
        `File exceeds limit of ${(MAX_FILE_BYTES / (1024 * 1024)).toFixed(
          0
        )}MB.`
      );
      return;
    ...
    if (!res.ok) {
      ...
      try {
        const data = await res.json();
        if (data && data.message) msg = data.message; // show only the message field
      } catch (e) {
  ...
  btnRecord.addEventListener("click", async () => {
    ...
    if (typeof MediaRecorder === "undefined") {
      alert("MediaRecorder API not available.");
      return;
    }
    ...
    if (liveHint) liveHint.textContent = `Recording… Max ${MAX_SECONDS}s.`;
    ...
    try {
      ...
      alert("Recorder cannot start with available codecs.");
      stopLiveStream();
      return;
    }
    ...
    mediaRecorder.onstop = async () => {
      ...
      const blob = new Blob(recordedChunks, {
        type: recordedMime || "video/webm",
      });
      if (blob.size > MAX_FILE_BYTES) {
        recordedChunks = [];
        alert(
          `Recorded file exceeds ${(MAX_FILE_BYTES / (1024 * 1024)).toFixed(
            0
          )}MB. Retake a shorter clip.`
        );
        return;
      }
      ...
      if (finalDuration > MAX_SECONDS + 0.5) {
        recordedChunks = [];
        alert(`Recorded video exceeds ${MAX_SECONDS}s. Please retake.`);
        return;
      }
      ...
      metaEl.textContent = `Duration ~ ${finalDuration.toFixed(1)}s · Size ${(
        blob.size /
        (1024 * 1024)
      ).toFixed(2)} MB`;
  ...
  function safeStopRecording() {
    ...
    // Do NOT force display of confirm row here; onstop controls that.
  }
  ...
  function estimateBlobDuration(blob) {
    return new Promise((resolve) => {
      ...
      v.preload = "metadata";
      v.onloadedmetadata = () => {
        const d = isFinite(v.duration) ? v.duration : MAX_SECONDS;
